...
- Constrained attribute values (variants, sizes, button types, HTTP methods, modal sizes) come from referential enums in `BladeUIKitBootstrap\Enums` — they are validated at render time (invalid value throws `InvalidArgumentException`) and documented via PHPDoc `@var`/`@param` on the component properties (PHP-level IntelliSense; current IDEs do not surface these inside `<x-...>` Blade tags).
- **IDE metadata**: `php artisan blade-ui-kit-bs:ide` generates `.vscode/blade-ui-kit-bootstrap.code-snippets` (VS Code snippets — zero install), `.vscode/blade-ui-kit-bootstrap.html-data.json` (VS Code Custom Data for a forthcoming dedicated extension), and `ide-helper/blade-ui-kit-bootstrap/ide.json` (PhpStorm / Laravel Idea alias→class mapping, auto-detected via recursive scan + merge). Respects the configured prefix and any merge/replace customisations.
