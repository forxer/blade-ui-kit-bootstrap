...
- **PhpStorm `ide.json`**: the file generated by `blade-ui-kit-bs:ide` was named
  `blade-ui-kit-bootstrap.ide.json` and written to `.vscode/`, so Laravel Idea never picked it
  up (it only reads files named exactly `ide.json`). It is now written as `ide.json` in a
  package-owned subfolder, `ide-helper/blade-ui-kit-bootstrap/`, which Laravel Idea
  auto-detects through its recursive scan and merges with any `ide.json` the application
...
- Added missing `show` and `hide` properties
